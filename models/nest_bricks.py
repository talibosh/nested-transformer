...
class NesTTransformerBrick(nn.Module):
    ...
    def setup(self):
        ...
        self.encoder_dict = dict(
            ...
            train=self.train,
            dtype=self.dtype)
        self.patch_embading_fn = self_attention.PatchEmbedding(
                patch_size=(self.config.init_patch_embed_size, self.config.init_patch_embed_size),
                embedding_dim=self.config.embedding_dim, conv_fn=self.conv_fn)

    def __call__(self, inputs, level):
        if level == 0:
            ins = {'params': 6}
            x = self.patch_embading_fn( inputs)
        else:
            x = inputs
        x = attn_utils.block_images(x, (self.config.patch_size, self.config.patch_size))
