...
def get_config():
  ...
  config.num_epochs = 1 #300
  config.num_train_steps = -1
  config.num_eval_steps = -1

  config.eval_pad_last_batch = True
  config.log_loss_every_steps = 5000 #3000
  config.eval_every_steps = -1
  config.eval_per_epochs = 1 #10
  ...
  #config.augment.type = 'default' #"randaugment"  # Set to `default` to disable
  ...
  #config.mix.prob_to_apply = 0 #1.0  # Set to 0 to disable
